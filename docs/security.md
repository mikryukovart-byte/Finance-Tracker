...
The following tables are server-only and intentionally have RLS enabled with no `anon` or `authenticated` policies: `DailyActionLog`, `TelegramPendingAction`, `TelegramPendingWorkRecord`, `WorkRecord`, `MonthlyTaktLevel`, `ThreeYearGoalScenario`, `AnnualGoalRow`, `AnnualGoalPlan`, `CrisisSettings`, and `WeeklyHypothesis`.
...
The server-only RLS rules are defined by [the RLS hardening migration](../prisma/migrations/20260702120000_harden_server_only_rls/migration.sql) and the Work Records migration. Run [the verification queries](sql/verify-rls.sql) in Supabase SQL Editor after security changes. The first and third queries must return no rows; every row in the second query must show `true`, `false`, `false`.
